...
| v1.6.1 | 2026-04-24 | **Flush Phase B/C hardening (field feedback).** spec §11.6 adds a normative "Phase B and Phase C are non-skippable" sentence: zero extractable items still requires an explicit empty-scan report; omitting Phase B is a flush failure indistinguishable from `sync`. Reinforced across flush.md, SKILL.md, operational_rules.md, and the Kimi skill. |
| v1.7.0 | 2026-04-24 | **Sixth command `/doc-harness resume`.** spec §11.7 becomes State Resume: explicit, verifiable session recovery — Recovery Chain execution, 7-section Recovery Report, 5-question Understanding Verification, interactive/auto modes with ≤7d / 8–30d / >30d decision tree; Phases B/C non-skippable. Information Recall renumbered to §11.8. New reference doc `resume.md` (EN/ZH/Kimi). |
| v1.7.1 | 2026-09-01 | **Packaging-only release (no normative spec change).** Repaired `skill-zh/SKILL.md` YAML frontmatter (unescaped double quotes broke skill discovery in Claude Code). Modernized plugin manifests: per-plugin `.claude-plugin/plugin.json` and marketplace `source` paths pointing at each plugin directory (the old inline format predates the current plugin schema). |
