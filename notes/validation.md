...
**对外口径协调**：一个下游项目来信询问 v2 的对外描述，回信给出了口径快照与**必须停用的 v1.7.x 说法清单**（「五个文档」「WORKLOG」「车身/阶段」停用；「六个命令」「恢复链」「MIT/双语/插件市场」保留）。当时的说法是「支持 Kimi CLI」须改口为「与 agent 解耦」——**这一条在 2026-09-18 被修正并强化**：Kimi CLI 的官方发现规则（品牌目录 `~/.kimi/skills/`、`~/.claude/skills/`、`~/.codex/skills/`，默认合并、同名按 kimi > claude > codex）意味着**同一个 skill 目录即装即可，根本不需要 Kimi 专用分支**；同时把仓库里那份 v1.6 专用分叉删除（它教的是已淘汰模型，对一个 Kimi 岗位反而是负资产）。细节与出处见 `notes/kimi-claude-interop.md`。
