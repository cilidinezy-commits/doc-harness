...
**Pushed 2026-09-01**: master 42f6aba + tags v1.4.1–v1.7.1 to GitHub; verified via `git ls-remote`. Claude plugin updated to 1.7.1 (restart to apply); Codex snapshot upgraded to 42f6aba and plugin shows 1.7.1; isolated-CODEX_HOME install test passed. Historical spec-history rows for v1.6.1/v1.7.0 backfilled; v1.4.1–v1.7.0 tags created; Kimi deployment verified in sync. Remaining user action: restart Claude Code to load the updated plugin.
